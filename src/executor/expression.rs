...
use crate::sql::ast::{BinaryOperator, DataType, Expression, UnaryOperator};
use crate::Value;
use rand::Rng;
...
//
// 采用「哨兵列名」方案：
// 1. 静态解析期（resolve_subqueries_in_expr）：已知外层作用域后，将子查询中
//    匹配外层列的 ColumnRef 改写为哨兵名 `__engram_outer_<key>`。
//    有改写 → 关联子查询 → 保留表达式推迟到运行时；无改写 → 非关联 →
//    立即静态求值为字面量（零运行时开销）。
// 2. 运行时（filter/projection 的 *_with_db 路径）：逐行构造外层行上下文
//    (列名, 值) 对，bind_outer_columns 将哨兵引用替换为该行的字面量，
//    再规划执行子查询。每行独立求解，正确处理关联语义。
// ============================================================================

/// 哨兵列名前缀：标识"此处实际是外层查询的某列值"
pub const OUTER_SENTINEL_PREFIX: &str = "__engram_outer_";

/// 外层作用域信息（静态解析期由物理计划收集）
#[derive(Debug, Clone, Default)]
pub struct OuterScope {
    /// 外层可见的输出列名（含 join 产生的 "t.c" 形式）
    pub cols: Vec<String>,
    /// 外层 FROM 引用的表名/别名
...
impl OuterScope {
    pub fn is_empty(&self) -> bool {
        self.cols.is_empty() && self.tables.is_empty()
    }
}

/// 将子查询中匹配外层作用域的 ColumnRef 改写为哨兵名
///
/// 返回 true 表示至少改写了一处（即这是一个关联子查询）。
/// 哨兵键取原始引用形态："t.c"（带限定）或 "c"（裸列名）。
pub fn normalize_outer_refs(
    stmt: &mut crate::sql::ast::SelectStmt,
    scope: &OuterScope,
    ...
    // 内层作用域遮蔽集：子查询 FROM 链上所有表名与 schema 列名
    let shadowed = collect_shadowed_columns(stmt);
    let mut changed = false;
    for item in stmt.select_list.iter_mut() {
        if let crate::sql::ast::SelectItem::Expression(e, _) = item {
            changed |= normalize_outer_refs_in_expr(e, scope, db, &shadowed);
        }
    }
    for gb in stmt.group_by.iter_mut() {
        changed |= normalize_outer_refs_in_expr(gb, scope, db, &shadowed);
    }
    if let Some(h) = stmt.having.as_mut() {
        changed |= normalize_outer_refs_in_expr(h, scope, db, &shadowed);
    }
    if let Some(w) = stmt.where_clause.as_mut() {
        changed |= normalize_outer_refs_in_expr(w, scope, db, &shadowed);
    }
    for ob in stmt.order_by.iter_mut() {
        changed |= normalize_outer_refs_in_expr(&mut ob.expr, scope, db, &shadowed);
    }
    changed
}

/// 递归收集子查询内部可见的表名与 schema 列名（用于裸列名遮蔽判定）
fn collect_shadowed_columns(stmt: &crate::sql::ast::SelectStmt) -> std::collections::HashSet<String> {
    fn walk_tr(tr: &crate::sql::ast::TableRef, out: &mut std::collections::HashSet<String>) {
    ...
    if let Some(from) = &stmt.from {
        walk_tr(from, &mut tables);
    }

    let mut cols = std::collections::HashSet::new();
    // 表达式级自引用列（本轮 where/select 等裸列若出现在表达式里通常就是内层列）
    let mut add_expr_cols = |e: &Expression| collect_column_names(e, &mut cols);
...
/// 临时：表达式裸列名收集
fn collect_column_names(e: &Expression, out: &mut std::collections::HashSet<String>) {
    match e {
        ...
        Expression::UnaryOp { expr, .. } => collect_column_names(expr, out),
        Expression::IsNull(x) | Expression::IsNotNull(x) => collect_column_names(x, out),
        Expression::Cast { expr, .. } => collect_column_names(expr, out),
        Expression::InList { expr, list } => {
            collect_column_names(expr, out);
            for i in list {
                collect_column_names(i, out);
            }
        }
        Expression::InSubquery { expr, .. } => collect_column_names(expr, out),
...
fn outer_key_matches(key: &str, table: &Option<String>, column: &str, scope: &OuterScope) -> bool {
    match table {
        Some(t) => {
            (scope.tables.iter().any(|x| x == t) && scope.cols.iter().any(|x| x == column))
                || scope.cols.iter().any(|x| x == key)
        }
        None => scope.cols.iter().any(|x| x == column),
    }
}

pub(crate) fn normalize_outer_refs_in_expr(
    e: &mut Expression,
    scope: &OuterScope,
    ...
    match e {
        Expression::ColumnRef { table, column } => {
            // 幂等：已是本模块注入的哨兵 → 视为关联（重复 resolve 时保持延迟语义）
            if table.is_none() && column.starts_with(OUTER_SENTINEL_PREFIX) {
                return true;
            }
            let key = match table {
            ...
            // 裸列名遮蔽保护：同名列属于内层作用域时不改写，
            // 仅当它不属于内层可见列集且命中外层输出列时才绑定
            let is_shadowed = table.is_none() && shadowed.contains(column);
            if !is_shadowed && outer_key_matches(&key, table, column, scope) {
                *column = format!("{}{}", OUTER_SENTINEL_PREFIX, key);
                *table = None;
                true
            } else {
                false
        ...
        Expression::BinaryOp { left, right, .. } => {
            normalize_outer_refs_in_expr(left, scope, _db, shadowed)
                | normalize_outer_refs_in_expr(right, scope, _db, shadowed)
        }
        Expression::UnaryOp { expr, .. } => normalize_outer_refs_in_expr(expr, scope, _db, shadowed),
        Expression::IsNull(inner) | Expression::IsNotNull(inner) => {
            normalize_outer_refs_in_expr(inner, scope, _db, shadowed)
        }
        Expression::Cast { expr, .. } => normalize_outer_refs_in_expr(expr, scope, _db, shadowed),
        Expression::InList { expr, list } => {
            let mut c = normalize_outer_refs_in_expr(expr, scope, _db, shadowed);
            for item in list.iter_mut() {
                c |= normalize_outer_refs_in_expr(item, scope, _db, shadowed);
            }
            c
        }
        Expression::InSubquery { expr, .. } => normalize_outer_refs_in_expr(expr, scope, _db, shadowed),
        Expression::Like { expr, pattern } => {
            normalize_outer_refs_in_expr(expr, scope, _db, shadowed)
                | normalize_outer_refs_in_expr(pattern, scope, _db, shadowed)
        }
        Expression::Case { when_then, else_expr } => {
            let mut c = false;
            for (w, t) in when_then.iter_mut() {
                c |= normalize_outer_refs_in_expr(w, scope, _db, shadowed);
                c |= normalize_outer_refs_in_expr(t, scope, _db, shadowed);
            }
            if let Some(el) = else_expr.as_mut() {
                c |= normalize_outer_refs_in_expr(el, scope, _db, shadowed);
            }
            c
        }
        Expression::Function { args, .. } => {
            ...
            for a in args.iter_mut() {
                c |= normalize_outer_refs_in_expr(a, scope, _db, shadowed);
            }
            c
        }
        _ => false,
...
/// 将哨兵引用替换为外部行实际值（运行时逐行调用）
///
/// 返回 true 表示至少绑定了一处哨兵；未发现任何哨兵时返回原样克隆。
pub fn bind_outer_columns(
    subquery: &crate::sql::ast::SelectStmt,
    outer_row: &[(String, Value)],
) -> crate::sql::ast::SelectStmt {
    use std::collections::HashMap;

    // 构建哨兵映射表：`__engram_outer_<key>` → 行值
    let bindings: HashMap<String, &Value> = outer_row
        .iter()
        .map(|(k, v)| (format!("{}{}", OUTER_SENTINEL_PREFIX, k), v))
        .collect();
    // 同时保留原始 key 视图，便于按原始引用查找（上游可能直接传 "t.c"/"c"）
    let raw: HashMap<&str, &Value> = outer_row.iter().map(|(k, v)| (k.as_str(), v)).collect();

    let mut sub = subquery.clone();
    #[cfg(feature = "never")]
    eprintln!(
        "DBG bind keys={:?} wc_has_sentinel={}",
        outer_row.iter().map(|(k, _)| k.clone()).collect::<Vec<_>>().join(","),
        sub.where_clause.as_ref().map(has_sentinel).unwrap_or(false)
    );

    for item in sub.select_list.iter_mut() {
        if let crate::sql::ast::SelectItem::Expression(e, _) = item {
            bind_sentinels_in_expr(e, &bindings, &raw);
        }
    }
    for gb in sub.group_by.iter_mut() {
        bind_sentinels_in_expr(gb, &bindings, &raw);
    }
    if let Some(h) = sub.having.as_mut() {
        bind_sentinels_in_expr(h, &bindings, &raw);
    }
    if let Some(w) = sub.where_clause.as_mut() {
        bind_sentinels_in_expr(w, &bindings, &raw);
    }

    sub
}

type BindMap<'a> = HashMap<String, &'a Value>;

/// 哨兵列查找：后缀与运行时键存在多种形态差异（限定/裸名）时的三级回退
///
/// 1. 后缀精确匹配（"t1.id" 或 "id" 均直接命中）
/// 2. 后缀含点：取点后裸名再试（"t1.id" → "id"）
/// 3. 裸名失败：任一键的最后一段等于后缀（"t1.id" 命中键 "t1.id"）
fn lookup_sentinel_value<'a>(column: &str, raw: &HashMap<&'a str, &'a Value>) -> Option<&'a Value> {
    let suffix = column.strip_prefix(OUTER_SENTINEL_PREFIX)?;
    if let Some(v) = raw.get(suffix) {
        return Some(v);
    }
    if let Some((_, bare)) = suffix.rsplit_once('.') {
        if let Some(v) = raw.get(bare) {
            return Some(v);
        }
    }
    for (k, v) in raw.iter() {
        let last_seg = k.rsplit('.').next().unwrap_or(k);
        if last_seg == suffix {
            return Some(*v);
        }
    }
...
fn bind_sentinels_in_expr<'a>(e: &mut Expression, sentinels: &BindMap<'a>, raw: &HashMap<&'a str, &'a Value>) {
    match e {
        Expression::ColumnRef { table: None, column } => {
            // 仅替换哨兵列（非哨兵的内层列一律不动，避免误绑定）
            if column.starts_with(OUTER_SENTINEL_PREFIX) {
                if let Some(v) = lookup_sentinel_value(column, raw).or_else(|| sentinels.get(column).copied()) {
                    *e = Expression::Literal(v.clone());
                }
            }
        }
        Expression::BinaryOp { left, right, .. } => {
            bind_sentinels_in_expr(left, sentinels, raw);
            bind_sentinels_in_expr(right, sentinels, raw);
        }
        Expression::UnaryOp { expr, .. } => bind_sentinels_in_expr(expr, sentinels, raw),
        Expression::IsNull(inner) | Expression::IsNotNull(inner) => bind_sentinels_in_expr(inner, sentinels, raw),
        Expression::Cast { expr, .. } => bind_sentinels_in_expr(expr, sentinels, raw),
        Expression::InList { expr, list } => {
            bind_sentinels_in_expr(expr, sentinels, raw);
            for item in list.iter_mut() {
                bind_sentinels_in_expr(item, sentinels, raw);
            }
        }
        Expression::InSubquery { expr, .. } => bind_sentinels_in_expr(expr, sentinels, raw),
        Expression::Like { expr, pattern } => {
            bind_sentinels_in_expr(expr, sentinels, raw);
            bind_sentinels_in_expr(pattern, sentinels, raw);
        }
        Expression::Case { when_then, else_expr } => {
            for (w, t) in when_then.iter_mut() {
                bind_sentinels_in_expr(w, sentinels, raw);
                bind_sentinels_in_expr(t, sentinels, raw);
            }
            if let Some(el) = else_expr.as_mut() {
                bind_sentinels_in_expr(el, sentinels, raw);
            }
        }
        Expression::Function { args, .. } => {
            for a in args.iter_mut() {
                bind_sentinels_in_expr(a, sentinels, raw);
            }
        }
        _ => {}
    }
...
pub fn contains_subquery(expr: &Expression) -> bool {
    match expr {
        Expression::Subquery(_) | Expression::Exists { .. } | Expression::InSubquery { .. } => true,
        ...
        Expression::IsNull(inner) | Expression::IsNotNull(inner) => contains_subquery(inner),
        ...
        Expression::Like { expr, pattern } => contains_subquery(expr) || contains_subquery(pattern),
        Expression::Case { when_then, else_expr } => {
            when_then.iter().any(|(w, t)| contains_subquery(w) || contains_subquery(t))
...
pub fn eval_vectorized(expr: &Expression, chunk: &DataChunk, column_names: &[String]) -> Result<Vector> {
    eval_vectorized_with_db(expr, chunk, column_names, None, None)
}
...
pub fn eval_vectorized_with_db(
    ...
    match expr {
        ...
        Expression::BinaryOp { left, op, right } => {
            let left_vec = eval_vectorized_with_db(left, chunk, column_names, db.as_deref_mut(), outer_row)?;
            let right_vec = eval_vectorized_with_db(right, chunk, column_names, db.as_deref_mut(), outer_row)?;
            eval_binary_vectorized(&left_vec, *op, &right_vec)
        }
        Expression::UnaryOp { op, expr } => {
            let vec = eval_vectorized_with_db(expr, chunk, column_names, db.as_deref_mut(), outer_row)?;
        ...
        Expression::InList { expr, list } => {
            ...
            for e in list {
                list_vecs.push(eval_vectorized_with_db(
                    e,
                    chunk,
                    column_names,
                    db.as_deref_mut(),
        ...
        Expression::Like { expr, pattern } => {
            let vec = eval_vectorized_with_db(expr, chunk, column_names, db.as_deref_mut(), outer_row)?;
            let pat_vec = eval_vectorized_with_db(pattern, chunk, column_names, db.as_deref_mut(), outer_row)?;
            eval_like(&vec, &pat_vec)
        }
        Expression::Case { when_then, else_expr } => {
            eval_case_vectorized_with_db(when_then, else_expr.as_deref(), chunk, column_names, db, outer_row)
        }
        Expression::Function { name, args, .. } => {
            eval_function_with_db(name, args, chunk, column_names, db, outer_row)
        }
        Expression::Placeholder(_) => Err(EngramDbError::Internal(
            "Placeholder should be resolved before execution".into(),
        )),
        // 标量子查询支持（v0.22.0 新增）
        Expression::Subquery(subquery) => {
            if let Some(mut db) = db {
                let value = eval_scalar_subquery(subquery, &mut db, outer_row)?;
                Ok(Vector::Constant(value, chunk.count))
            } else {
                Err(EngramDbError::Internal("Subquery requires database context".into()))
            }
        }
        // EXISTS 子查询支持（v0.22.0 新增）
        Expression::Exists { subquery, negated } => {
            if let Some(mut db) = db {
                let value = eval_exists_subquery(subquery, &mut db, *negated, outer_row)?;
                Ok(Vector::Constant(value, chunk.count))
            } else {
                Err(EngramDbError::Internal("Subquery requires database context".into()))
            }
        ...
        Expression::InSubquery {
            ...
            if let Some(mut db) = db {
                let expr_vec = eval_vectorized_with_db(expr, chunk, column_names, Some(&mut db), outer_row)?;
                ...
                let result = eval_in_subquery(&expr_value, subquery, &mut db, *negated, outer_row)?;
...
pub fn eval_scalar_subquery(
    ...
    // 如果有外部行，注入到子查询 WHERE 子句中
    let subquery = if let Some(row) = outer_row {
        bind_outer_columns(subquery, row)
    } else {
        subquery.clone()
    };
    ...
    if subquery.from.is_none() {
        return Err(EngramDbError::Parse("Scalar subquery must have a FROM clause".into()));
    }
    ...
    if result.rows.is_empty() {
        ...
        if row.is_empty() {
            // 行存在但无列（异常情况）
            return Err(EngramDbError::Internal("Subquery returned row with no columns".into()));
        }
        // 取第一列的值
        Ok(row[0].clone())
    }
...
pub fn eval_exists_subquery(
    ...
    // 验证子查询语句
    if subquery.from.is_none() {
        return Err(EngramDbError::Parse("EXISTS subquery must have a FROM clause".into()));
    }
...
fn expr_dbg_has_sentinel(e: &Expression) -> bool {
    match e {
        Expression::ColumnRef { column, .. } => column.starts_with(OUTER_SENTINEL_PREFIX),
        Expression::BinaryOp { left, right, .. } => expr_dbg_has_sentinel(left) || expr_dbg_has_sentinel(right),
        _ => false,
    }
}
...
pub fn eval_in_subquery(
    ...
    // 验证子查询语句
    if subquery.from.is_none() {
        return Err(EngramDbError::Parse("IN subquery must have a FROM clause".into()));
    }
    ...
    let found = result
        .rows
        .iter()
        .any(|row| if row.is_empty() { false } else { row[0] == *expr_value });
...
use crate::common::column_data::{BitVec, ColumnData, ColumnValue};
...
fn eval_typed_binary(l: &ColumnData, op: BinaryOperator, r: &ColumnData) -> Option<Result<Vector>> {
    use BinaryOperator::*;
    use ColumnValue::{Boolean, Float32, Float64, Int64, Timestamp, Varchar};
    ...
    let result: ColumnData = match (&col.values, c) {
        (Int64(v), Value::Int32(_)) | (Int64(v), Value::Int64(_)) | (Int64(v), Value::Timestamp(_)) => {
            ...
            match op {
                Plus | Minus | Multiply | Divide | Modulo => {
                    ...
                    ColumnData {
                        values: ColumnValue::Int64(out.0),
                        nulls: out.1,
                    }
                }
                Eq | NotEq | Lt | LtEq | Gt | GtEq => {
                    ...
                    ColumnData {
                        values: ColumnValue::Boolean(out.0),
                        nulls: out.1,
                    }
                }
                _ => return None,
        ...
        (Float64(v), Value::Int32(_)) | (Float64(v), Value::Int64(_)) | (Float64(v), Value::Float64(_)) => {
            ...
            match op {
                Plus | Minus | Multiply | Divide | Modulo => {
                    ...
                    ColumnData {
                        values: ColumnValue::Float64(out.0),
                        nulls: out.1,
                    }
                }
                Eq | NotEq | Lt | LtEq | Gt | GtEq => {
        ...
        (Varchar(v), Value::Varchar(cv)) => match op {
            ...
            Concat => {
                ...
                ColumnData {
                    values: ColumnValue::Varchar(out.0),
                    nulls: out.1,
                }
            }
            _ => return None,
...
fn merge_nulls(ln: &Option<BitVec>, rn: &Option<BitVec>, len: usize) -> Option<BitVec> {
    match (ln, rn) {
        ...
        _ => {
            ...
            let mut any = false;
            for i in 0..len {
                let is_null = ln.as_ref().map_or(false, |b| b.test(i)) || rn.as_ref().map_or(false, |b| b.test(i));
                if is_null {
                    bv.set(i, true);
                    any = true;
                }
...
fn merge_nulls_with(ln: &Option<BitVec>, rn: &Option<BitVec>, overflow: &[bool], len: usize) -> Option<BitVec> {
    ...
    for i in 0..len {
        let is_null =
            ln.as_ref().map_or(false, |b| b.test(i)) || rn.as_ref().map_or(false, |b| b.test(i)) || overflow[i];
        if is_null {
            bv.set(i, true);
            any = true;
        }
...
fn arith_typed_f64(la: &[f64], ra: &[f64], op: BinaryOperator, ln: &Option<BitVec>, rn: &Option<BitVec>) -> ColumnData {
    ...
    ColumnData {
        values: ColumnValue::Float64(out),
        nulls,
    }
}
...
fn cmp_typed_i64(la: &[i64], ra: &[i64], op: BinaryOperator, ln: &Option<BitVec>, rn: &Option<BitVec>) -> ColumnData {
    ...
    ColumnData {
        values: ColumnValue::Boolean(out),
        nulls,
    }
}
...
fn concat_typed(la: &[String], ra: &[String], ln: &Option<BitVec>, rn: &Option<BitVec>) -> ColumnData {
    ...
    ColumnData {
        values: ColumnValue::Varchar(out),
        nulls,
    }
}
...
fn eval_unary_vectorized(vec: &Vector, op: UnaryOperator) -> Result<Vector> {
    ...
    match &vec {
        ...
        Vector::Flat(values) => {
            let result: Vec<Value> = values.iter().map(|v| eval_unary_value(v, op)).collect();
            Ok(Vector::Flat(result))
        }
    }
}
...
/// 公开的单值转换入口（供 INSERT 强转使用；失败语义：返回 NULL）
pub fn cast_value_public(v: &Value, target: &DataType) -> Value {
    cast_value(v, target).unwrap_or(Value::Null)
}
    ...
    Ok(match target {
        ...
        Int16 => {
            if let Some(i) = v.as_i64() {
                ...
                s.parse::<i16>().map(Value::Int16).unwrap_or(Value::Null)
            } else {
                Value::Null
            }
        }
        Decimal { scale, .. } => {
            ...
            if let Some(i) = v.as_i64() {
                ...
                match crate::common::value_cmp::parse_decimal_str(s, *scale) {
                    Some(n) => Value::Decimal(n, *scale),
                    None => Value::Null,
                }
            } else {
                Value::Null
        ...
        Varchar => Value::Varchar(format!("{}", v)),
        _ => Value::Null,
    })
}
...
fn eval_in_list(expr_vec: &Vector, list_vecs: &[Vector]) -> Result<Vector> {
    ...
    let list_values: Vec<Value> = list_vecs
        .iter()
    ...
    match &expr_vec {
        ...
        Vector::Flat(values) => {
            if all_constant {
                ...
                let result: Vec<Value> = values.iter().map(|v| Value::Boolean(set.contains(v))).collect();
                Ok(Vector::Flat(result))
            } else {
                // 通用情况
                let mut result = Vec::with_capacity(values.len());
...
fn eval_case_vectorized(
    ...
    eval_case_vectorized_with_db(when_then, else_expr, chunk, column_names, None, None)
}
...
fn eval_case_vectorized_with_db(
    ...
    for (when_expr, then_expr) in when_then {
        let when_vec = eval_vectorized_with_db(when_expr, chunk, column_names, db.as_deref_mut(), outer_row)?;
        let then_vec = eval_vectorized_with_db(then_expr, chunk, column_names, db.as_deref_mut(), outer_row)?;
        when_vecs.push(when_vec);
        then_vecs.push(then_vec);
    }

    let else_vec = match else_expr {
        Some(e) => eval_vectorized_with_db(e, chunk, column_names, db.as_deref_mut(), outer_row)?,
...
/// 辅助宏：简化 eval_vectorized 调用（自动传递 db 和 outer_row 参数）
macro_rules! eval {
    ($expr:expr, $chunk:expr, $names:expr, $db:expr, $outer:expr) => {
        eval_vectorized_with_db($expr, $chunk, $names, $db.as_deref_mut(), $outer)
    };
}

fn eval_function(name: &str, args: &[Expression], chunk: &DataChunk, column_names: &[String]) -> Result<Vector> {
    eval_function_with_db(name, args, chunk, column_names, None, None)
}
...
fn eval_function_with_db(
    ...
    match func_name.as_str() {
        "ABS" => {
            ...
            let vec = eval!(&args[0], chunk, column_names, db, outer_row)?;
            eval_abs(&vec)
        }
        "LENGTH" | "LEN" => {
            if args.len() != 1 {
        ...
        "COALESCE" => {
            ...
            for a in args {
                arg_vecs.push(eval_vectorized_with_db(
                    a,
                    chunk,
                    column_names,
                    db.as_deref_mut(),
        ...
        "SUBSTRING" | "SUBSTR" => {
            ...
            let str_vec = eval!(&args[0], chunk, column_names, db, outer_row)?;
            let start_vec = eval!(&args[1], chunk, column_names, db, outer_row)?;
            let len_vec = if args.len() == 3 {
                Some(eval!(&args[2], chunk, column_names, db, outer_row)?)
            } else {
                None
            };
        ...
        "NULLIF" => {
            ...
            let a_vec = eval!(&args[0], chunk, column_names, db, outer_row)?;
            let b_vec = eval!(&args[1], chunk, column_names, db, outer_row)?;
        ...
        "IF" => {
            ...
            let cond_vec = eval!(&args[0], chunk, column_names, db, outer_row)?;
            let true_vec = eval!(&args[1], chunk, column_names, db, outer_row)?;
            let false_vec = eval!(&args[2], chunk, column_names, db, outer_row)?;
        ...
        "TRIM" => {
            ...
            let chars_vec = if args.len() == 2 {
                Some(eval!(&args[1], chunk, column_names, db, outer_row)?)
            } else {
                None
            };
        ...
        "INSTR" | "POSITION" => {
            ...
            let haystack_vec = eval!(&args[0], chunk, column_names, db, outer_row)?;
            let needle_vec = eval!(&args[1], chunk, column_names, db, outer_row)?;
        ...
        "SPLIT_PART" => {
            ...
            let str_vec = eval!(&args[0], chunk, column_names, db, outer_row)?;
            let delim_vec = eval!(&args[1], chunk, column_names, db, outer_row)?;
            let part_vec = eval!(&args[2], chunk, column_names, db, outer_row)?;
        ...
        "POWER" | "POW" => {
            ...
            let base_vec = eval!(&args[0], chunk, column_names, db, outer_row)?;
            let exp_vec = eval!(&args[1], chunk, column_names, db, outer_row)?;
        ...
        "REPLACE" => {
            ...
            let str_vec = eval!(&args[0], chunk, column_names, db, outer_row)?;
            let from_vec = eval!(&args[1], chunk, column_names, db, outer_row)?;
            let to_vec = eval!(&args[2], chunk, column_names, db, outer_row)?;
        ...
        "JSON_EXTRACT" => {
            ...
            let json_vec = eval!(&args[0], chunk, column_names, db, outer_row)?;
            let path_vec = eval!(&args[1], chunk, column_names, db, outer_row)?;
        ...
        "JSON_CONTAINS" => {
            ...
            let json_vec = eval!(&args[0], chunk, column_names, db, outer_row)?;
            let target_vec = eval!(&args[1], chunk, column_names, db, outer_row)?;
        ...
        "JSON_OBJECT" => {
            if args.len() % 2 != 0 {
                return Err(EngramDbError::Parse(
                    "JSON_OBJECT requires even number of arguments (key, value pairs)".into(),
                ));
            }
            ...
            for a in args.iter().step_by(2) {
                key_vecs.push(eval_vectorized_with_db(
                    a,
                    chunk,
                    column_names,
                    db.as_deref_mut(),
            ...
            for a in args.iter().skip(1).step_by(2) {
                val_vecs.push(eval_vectorized_with_db(
                    a,
                    chunk,
                    column_names,
                    db.as_deref_mut(),
        ...
        "JSON_SET" => {
            ...
            let json_vec = eval!(&args[0], chunk, column_names, db, outer_row)?;
            let path_vec = eval!(&args[1], chunk, column_names, db, outer_row)?;
            let val_vec = eval!(&args[2], chunk, column_names, db, outer_row)?;
        ...
        "VECTOR_DISTANCE" | "VEC_DISTANCE" => {
            ...
            let v1 = eval!(&args[0], chunk, column_names, db, outer_row)?;
            let v2 = eval!(&args[1], chunk, column_names, db, outer_row)?;
        ...
        "VECTOR_COSINE_SIMILARITY" => {
            if args.len() != 2 {
                return Err(EngramDbError::Parse(
                    "VECTOR_COSINE_SIMILARITY requires 2 arguments".into(),
                ));
            }
            let v1 = eval!(&args[0], chunk, column_names, db, outer_row)?;
            let v2 = eval!(&args[1], chunk, column_names, db, outer_row)?;
        ...
        "VECTOR_NORM" => {
            ...
            let v = eval!(&args[0], chunk, column_names, db, outer_row)?;
            eval_vector_norm(&v)
        }
        ...
        "STRFTIME" => {
            if args.len() != 2 {
                return Err(EngramDbError::Parse(
                    "STRFTIME requires 2 arguments (format, timestamp)".into(),
                ));
            }
            let fmt_vec = eval!(&args[0], chunk, column_names, db, outer_row)?;
            let ts_vec = eval!(&args[1], chunk, column_names, db, outer_row)?;
        ...
        "DATE_ADD" => {
            if args.len() != 3 {
                return Err(EngramDbError::Parse(
                    "DATE_ADD requires 3 arguments (timestamp, number, unit)".into(),
                ));
            }
            let ts_vec = eval!(&args[0], chunk, column_names, db, outer_row)?;
            let n_vec = eval!(&args[1], chunk, column_names, db, outer_row)?;
            let unit_vec = eval!(&args[2], chunk, column_names, db, outer_row)?;
            ...
            match (&ts_vec, &n_vec) {
                ...
                (Vector::Flat(ts), Vector::Flat(num)) => {
                    let result: Vec<Value> =
                        ts.iter().zip(num.iter()).map(|(t, n)| date_add_value(t, n, &unit)).collect();
                    Ok(Vector::Flat(result))
                }
        ...
        "DATE_SUB" => {
            if args.len() != 3 {
                return Err(EngramDbError::Parse(
                    "DATE_SUB requires 3 arguments (timestamp, number, unit)".into(),
                ));
            }
            let ts_vec = eval!(&args[0], chunk, column_names, db, outer_row)?;
            let n_vec = eval!(&args[1], chunk, column_names, db, outer_row)?;
            ...
            match (&ts_vec, &n_vec) {
                ...
                (Vector::Flat(ts), Vector::Flat(num)) => {
                    let result: Vec<Value> =
                        ts.iter().zip(num.iter()).map(|(t, n)| date_sub_value(t, n, &unit)).collect();
                    Ok(Vector::Flat(result))
                }
                _ => Ok(Vector::Flat(vec![Value::Null; chunk.count])),
            }
        ...
        "DATE_DIFF" => {
            if args.len() != 3 {
                return Err(EngramDbError::Parse(
                    "DATE_DIFF requires 3 arguments (timestamp1, timestamp2, unit)".into(),
                ));
            }
            let ts1_vec = eval!(&args[0], chunk, column_names, db, outer_row)?;
            let ts2_vec = eval!(&args[1], chunk, column_names, db, outer_row)?;
            let unit_vec = eval!(&args[2], chunk, column_names, db, outer_row)?;
            let unit = match &unit_vec {
                Vector::Constant(Value::Varchar(s), _) => s.to_lowercase(),
            ...
            match (&ts1_vec, &ts2_vec) {
                ...
                (Vector::Flat(ts1), Vector::Flat(ts2)) => {
                    let result: Vec<Value> =
                        ts1.iter().zip(ts2.iter()).map(|(a, b)| date_diff_value(a, b, &unit)).collect();
                    Ok(Vector::Flat(result))
                }
                _ => Ok(Vector::Flat(vec![Value::Null; chunk.count])),
            }
        ...
        "DATE_TRUNC" | "DATE_BIN" => {
            ...
            let unit_vec = eval!(&args[0], chunk, column_names, db, outer_row)?;
            let ts_vec = eval!(&args[1], chunk, column_names, db, outer_row)?;
            let unit = match &unit_vec {
                Vector::Constant(Value::Varchar(s), _) => s.to_lowercase(),
        ...
        "STRPTIME" => {
            if args.len() != 2 {
                return Err(EngramDbError::Parse(
                    "STRPTIME requires 2 arguments (format, string)".into(),
                ));
            }
            let fmt_vec = eval!(&args[0], chunk, column_names, db, outer_row)?;
            let s_vec = eval!(&args[1], chunk, column_names, db, outer_row)?;
            let fmt = match &fmt_vec {
                Vector::Constant(Value::Varchar(s), _) => s.clone(),
                _ => return Err(EngramDbError::Parse("STRPTIME format must be a constant string".into())),
            };
            let s_vec_norm = match s_vec {
                Vector::Typed(d) => Vector::Flat(d.to_values()),
                other => other.clone(),
            };
            match &s_vec_norm {
        ...
        "MATCH" => {
            if args.len() != 2 {
                return Err(EngramDbError::Parse(
                    "MATCH requires 2 arguments (column, query)".into(),
                ));
            }
            let vec = eval!(&args[0], chunk, column_names, db, outer_row)?;
            let query_vec = eval!(&args[1], chunk, column_names, db, outer_row)?;
        ...
        "DATE_TO_STRING" | "DATE_FORMAT" => {
            ...
            let date_vec = eval!(&args[0], chunk, column_names, db, outer_row)?;
            let fmt = if args.len() == 2 {
                match eval!(&args[1], chunk, column_names, db, outer_row)? {
        ...
        "TIME_TO_STRING" | "TIME_FORMAT" => {
            ...
            let time_vec = eval!(&args[0], chunk, column_names, db, outer_row)?;
            let fmt = if args.len() == 2 {
                match eval!(&args[1], chunk, column_names, db, outer_row)? {
                    Vector::Constant(Value::Varchar(s), _) => Some(s),
                    _ => None,
        ...
        "TIMESTAMP_TO_STRING" => {
            if args.len() < 1 || args.len() > 2 {
                return Err(EngramDbError::Parse(
                    "TIMESTAMP_TO_STRING requires 1-2 arguments".into(),
                ));
            }
            let ts_vec = eval!(&args[0], chunk, column_names, db, outer_row)?;
            let fmt = if args.len() == 2 {
        ...
        "UUID_TO_STRING" => {
            ...
            let uuid_vec = eval!(&args[0], chunk, column_names, db, outer_row)?;
        ...
        _ => Err(EngramDbError::Parse(format!("Unknown function: {}", name))),
    }
}
...
fn time_value(v: &Value) -> Value {
    ...
    if let Some(ts) = v.as_i64() {
        let secs = ts / 1000;
        let day_secs = if secs >= 0 {
            secs % 86400
        } else {
            (secs % 86400 + 86400) % 86400
        };
        let h = day_secs / 3600;
...
fn timestamp_to_string_value(v: &Value, _fmt: Option<&str>) -> Value {
    match v {
        Value::Null => Value::Null,
        Value::Timestamp(ts) => Value::Varchar(format_datetime(*ts)),
        Value::Int64(ts) => Value::Varchar(format_datetime(*ts)),
        _ => Value::Null,
    }
}
...
fn uuid_to_string_value(v: &Value) -> Value {
    match v {
        Value::Null => Value::Null,
        Value::Uuid(u) => Value::Varchar(format!(
...
fn strptime_value(fmt: &str, v: &Value) -> Value {
    ...
    if fmt.contains("%Y") && fmt.contains("%m") && fmt.contains("%d") {
        ...
        let digits: String = s
            .chars()
            .filter(|c| c.is_ascii_digit() || *c == '-' || *c == 'T' || *c == ':' || *c == ' ')
            .collect();
        ...
        if parts.len() >= 3 {
            ...
            let min = if parts.len() > 4 {
                parts[4].parse::<i64>().unwrap_or(0)
            } else {
                0
            };
            let sec = if parts.len() > 5 {
            ...
            Some(chars_str) => {
                ...
                let (start, end) = match mode {
                    TrimMode::Both => {
                        ...
                        while lo < hi && chars.contains(&s_chars[lo]) {
                            lo += 1;
                        }
                        while hi > lo && chars.contains(&s_chars[hi - 1]) {
                            hi -= 1;
                        }
                        (lo, hi)
                    }
                    TrimMode::Left => {
                        let mut lo = 0usize;
                        while lo < s_chars.len() && chars.contains(&s_chars[lo]) {
                            lo += 1;
                        }
                        (lo, s_chars.len())
                    }
                    TrimMode::Right => {
                        let mut hi = s_chars.len();
                        while hi > 0 && chars.contains(&s_chars[hi - 1]) {
                            hi -= 1;
                        }
                        (0, hi)
                    }
...
fn eval_binary_numeric<F: Fn(f64, f64) -> f64>(a_vec: &Vector, b_vec: &Vector, f: F) -> Result<Vector> {
    let len = a_vec.len();
    let mut result = Vec::with_capacity(len);
    for i in 0..len {
        let a = a_vec.get(i);
...
pub fn boolean_to_selection(bool_vec: &Vector) -> Vec<usize> {
    ...
    match bool_vec {
        Vector::Constant(Value::Boolean(true), n) => (0..*n).collect(),
        Vector::Constant(_, _) => {
            Vec::new() // false 或 NULL → 全不选
        }
        Vector::Flat(values) => values
            .iter()
            .enumerate()
...
fn eval_json_extract(json_vec: &Vector, path_vec: &Vector) -> Result<Vector> {
    match (json_vec, path_vec) {
        (Vector::Constant(jv, n), Vector::Constant(pv, _)) => Ok(Vector::Constant(json_extract_value(jv, pv), *n)),
        _ => {
...
fn eval_json_contains(json_vec: &Vector, target_vec: &Vector, path_vec: Option<&Vector>) -> Result<Vector> {
    ...
    for i in 0..len {
        let jv = &json_flat[i];
        let tv = if target_flat.len() == 1 {
            &target_flat[0]
        } else {
            &target_flat[i]
        };
        let pv = path_flat.as_ref().map(|p| if p.len() == 1 { &p[0] } else { &p[i] });
...
fn json_contains_recursive(container: &serde_json::Value, target: &serde_json::Value) -> bool {
    ...
    match container {
        serde_json::Value::Object(map) => map.values().any(|v| json_contains_recursive(v, target)),
        serde_json::Value::Array(arr) => {
            if target.is_array() {
                // 数组包含：target 的每个元素都在 container 数组中找到
                if let Some(target_arr) = target.as_array() {
                    return target_arr.iter().all(|t| arr.iter().any(|c| json_contains_recursive(c, t)));
                }
            }
...
fn value_to_json(v: &Value) -> serde_json::Value {
    match v {
        ...
        // v0.22.0 新增类型
        Value::Jsonb(s) => serde_json::from_str(s).unwrap_or_else(|_| serde_json::Value::String(s.clone())),
        Value::Decimal(v, s) => {
            if *s == 0 {
                ...
                serde_json::Value::Number(
                    serde_json::Number::from_f64(scaled).unwrap_or(serde_json::Number::from(0u64)),
                )
            }
        }
...
fn eval_vector_distance(v1: &Vector, v2: &Vector) -> Result<Vector> {
    match (v1, v2) {
        (Vector::Constant(a, n), Vector::Constant(b, _)) => Ok(Vector::Constant(vector_l2_distance(a, b), *n)),
        _ => {
            let f1 = v1.to_flat();
            let f2 = v2.to_flat();
            let len = f1.len();
...
fn eval_vector_cosine_similarity(v1: &Vector, v2: &Vector) -> Result<Vector> {
    match (v1, v2) {
        (Vector::Constant(a, n), Vector::Constant(b, _)) => Ok(Vector::Constant(vector_cosine_sim(a, b), *n)),
        _ => {
            let f1 = v1.to_flat();
            let f2 = v2.to_flat();
            let len = f1.len();
...
mod tests {
    ...
    fn test_literal_eval() {
        ...
        let result = eval_vectorized(&Expression::Literal(Value::Int64(42)), &chunk, &["col1".to_string()]).unwrap();
    ...
    fn test_is_null() {
        ...
        let expr = Expression::IsNull(Box::new(Expression::ColumnRef {
            table: None,
            column: "col1".to_string(),
        }));
        let result = eval_vectorized(&expr, &chunk, &["col1".to_string()]).unwrap();
    ...
    fn test_json_extract_null() {
        ...
        let result = json_extract_value(
            &Value::Json("{}".to_string()),
            &Value::Varchar("$.nonexistent".to_string()),
        );
        assert_eq!(result, Value::Null);
    }
    ...
    fn test_mod_basic() {
        let chunk = DataChunk {
            columns: vec![Vector::Flat(vec![Value::Int64(10), Value::Int64(7), Value::Int64(100)])],
            count: 3,
        };
        let expr = func_expr(
            "MOD",
    ...
    fn test_specialized_deterministic() {
        ...
        assert_eq!(
            out.to_flat(),
            vec![Value::Varchar("ab".into()), Value::Varchar("ba".into())]
        );
    ...
    fn test_specialized_matches_pair_random() {
        ...
        for trial in 0..800 {
            ...
            let expected: Vec<Value> = (0..l.len().min(r.len())).map(|i| eval_binary_pair(&l[i], op, &r[i])).collect();

            let specialized = try_eval_specialized(&Vector::Flat(l.clone()), op, &Vector::Flat(r.clone()));
            match specialized {
                ...
                None => {
                    ...
                    let via_vec =
                        eval_binary_vectorized(&Vector::Flat(l.clone()), op, &Vector::Flat(r.clone())).unwrap();
                    let Vector::Flat(out) = via_vec else {
                        panic!("expected flat")
                    };
                    for (a, b) in out.iter().zip(expected.iter()) {
                        assert!(
    ...
    fn bench_typed_vs_flat() {
        ...
        for _ in 0..3 {
            ...
            let out = eval_binary_vectorized(
                &Vector::Typed(ld.clone()),
                BinaryOperator::Plus,
                &Vector::Typed(rd.clone()),
            )
            .unwrap();
            t += t0.elapsed();
            assert_eq!(out.len(), n);
        ...
        for _ in 0..3 {
            let t0 = Instant::now();
            let out = eval_binary_vectorized(&Vector::Flat(l.clone()), BinaryOperator::Plus, &Vector::Flat(r.clone()))
                .unwrap();
            t += t0.elapsed();
            assert_eq!(out.len(), n);
        }
        ...
        println!(
            "1M 行 Int64+Int64: typed = {:?}, flat = {:?}, typed 快 {}x",
            typed_time,
            flat_time,
    ...
    fn test_typed_constant_matches_flat() {
        ...
        for trial in 0..300 {
            ...
            let typed_out =
                eval_binary_vectorized(&Vector::Typed(data.clone()), op, &Vector::Constant(constant.clone(), n))
                    .unwrap();
    ...
    fn test_typed_constant_reversed() {
        ...
        assert_eq!(
            out.to_flat(),
            vec![Value::Varchar("a!".into()), Value::Varchar("c!".into())]
        );
    }
    ...
    fn bench_specialized_vs_pair() {
        ...
        for _ in 0..3 {
            let t0 = Instant::now();
            let out = try_eval_specialized(&Vector::Flat(l.clone()), BinaryOperator::Plus, &Vector::Flat(r.clone()))
                .unwrap()
                .unwrap();
            t += t0.elapsed();
            assert_eq!(out.len(), n);
        ...
        println!(
            "1M 行 Int64+Int64: specialized = {:?}, old(含 to_flat) = {:?}, 快 {}x",
            spec_time,
            old_time,
        ...
        let ls: Vec<Value> = (0..n)
            .map(|_| Value::Varchar(format!("key_{}", rng.gen_range(0..1000))))
            .collect();
        let rs: Vec<Value> = (0..n)
            .map(|_| Value::Varchar(format!("key_{}", rng.gen_range(0..1000))))
            .collect();
        ...
        for _ in 0..3 {
            let t0 = Instant::now();
            let out = try_eval_specialized(&Vector::Flat(ls.clone()), BinaryOperator::Lt, &Vector::Flat(rs.clone()))
                .unwrap()
                .unwrap();
            t += t0.elapsed();
            assert_eq!(out.len(), n);
        ...
        println!(
            "1M 行 Varchar<Varchar: specialized = {:?}, old(含 to_flat) = {:?}, 快 {}x",
            spec_s,
            old_s,
    ...
    fn test_typed_binary_matches_flat_random() {
        ...
        for trial in 0..4000 {
            ...
            let Some(ld) = ColumnData::try_from_values(&lv) else {
                continue;
            };
            let Some(rd) = ColumnData::try_from_values(&rv) else {
                continue;
            };
            let typed_out = eval_binary_vectorized(&Vector::Typed(ld), op, &Vector::Typed(rd)).unwrap();
            let typed_vals = typed_out.to_flat();

            // Flat 参考路径
            let flat_out = eval_binary_vectorized(&Vector::Flat(lv.clone()), op, &Vector::Flat(rv.clone())).unwrap();
            let flat_vals = flat_out.to_flat();
