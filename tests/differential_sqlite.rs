...
fn eng_rows(conn: &mut Connection, sql: &str) -> Vec<Vec<String>> {
    let r = conn.execute(sql).unwrap_or_else(|e| panic!("EngramDB 执行失败: {}\n{}", sql, e));
    r.rows.iter().map(|row| row.iter().map(eng_cell).map(|c| norm(&c)).collect()).collect()
}
...
fn setup(eng: &mut Connection, lite: &rusqlite::Connection) {
    eng.execute("CREATE TABLE users (id INT64 PRIMARY KEY, name VARCHAR, age INT, city VARCHAR)").unwrap();
    eng.execute("CREATE TABLE orders (oid INT64 PRIMARY KEY, uid INT, amount INT)").unwrap();
    lite.execute_batch(
...
fn diff_projection_and_filter() {
    ...
    assert_same("projection subset", &mut eng, &lite, "SELECT name, age FROM users WHERE id = 10");
    assert_same("projection reorder", &mut eng, &lite, "SELECT age, name FROM users WHERE id = 10");
    assert_same("where gt", &mut eng, &lite, "SELECT id FROM users WHERE age > 50 ORDER BY id");
    assert_same("where and or", &mut eng, &lite, "SELECT id FROM users WHERE age >= 40 AND age <= 45 OR id = 1 ORDER BY id");
...
fn diff_order_limit() {
    let (mut eng, lite) = fresh();
    assert_same("order desc limit", &mut eng, &lite, "SELECT id, age FROM users ORDER BY age DESC LIMIT 5");
...
fn diff_aggregates() {
    ...
    assert_same("count where", &mut eng, &lite, "SELECT COUNT(*) FROM users WHERE age > 40");
    ...
    assert_same("group by", &mut eng, &lite, "SELECT city, COUNT(*) FROM users GROUP BY city ORDER BY city");
    assert_same("group by agg", &mut eng, &lite, "SELECT uid, SUM(amount), COUNT(*) FROM orders GROUP BY uid ORDER BY uid");
    assert_same("group by having", &mut eng, &lite, "SELECT city, COUNT(*) FROM users GROUP BY city HAVING COUNT(*) > 10 ORDER BY city");
    assert_same("empty agg", &mut eng, &lite, "SELECT COUNT(*), SUM(amount) FROM orders WHERE amount > 10000");
}

#[test]
fn diff_joins() {
    let (mut eng, lite) = fresh();
    assert_same("inner join", &mut eng, &lite,
        "SELECT u.name, o.amount FROM users u JOIN orders o ON u.id = o.uid WHERE o.uid = 5 ORDER BY o.amount");
    assert_same("inner join agg", &mut eng, &lite,
        "SELECT u.name, SUM(o.amount) FROM users u JOIN orders o ON u.id = o.uid GROUP BY u.name ORDER BY u.name");
    assert_same("left join null fill", &mut eng, &lite,
...
fn diff_subqueries() {
    let (mut eng, lite) = fresh();
    assert_same("scalar subquery", &mut eng, &lite,
        "SELECT name, (SELECT MAX(amount) FROM orders) FROM users WHERE id = 3");
    assert_same("in subquery", &mut eng, &lite,
...
fn diff_null_semantics() {
    ...
    lite.execute_batch("INSERT INTO users VALUES (100, 'nullage', NULL, '北京')").unwrap();

    assert_same("is null", &mut eng, &lite, "SELECT id FROM users WHERE age IS NULL");
    assert_same("is not null count", &mut eng, &lite, "SELECT COUNT(*) FROM users WHERE age IS NOT NULL");
    assert_same("null comparison empty", &mut eng, &lite, "SELECT id FROM users WHERE age = NULL");
    assert_same("null in agg skipped", &mut eng, &lite, "SELECT COUNT(age), SUM(age) FROM users WHERE id >= 95");
    assert_same("null in order", &mut eng, &lite, "SELECT id, age FROM users WHERE id >= 99 ORDER BY age, id");
}
...
fn diff_dml_roundtrip() {
    ...
    assert_same("after update", &mut eng, &lite, "SELECT id, age FROM users WHERE age = 99 ORDER BY id");
    ...
    assert_same("after delete rows", &mut eng, &lite, "SELECT oid, amount FROM orders WHERE amount >= 100 ORDER BY oid");
}
