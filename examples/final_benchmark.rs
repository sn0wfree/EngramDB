...
use engramdb::storage::{column_store::CompressionStats, Database};
...
fn bench_write_performance(n_rows: usize) {
    ...
    println!(
        "  底层API写入:      {:>8.2} ms  ({:>10.0} rows/s)  [{:.1}x]",
        dur_raw,
...
fn bench_compression_ratio(n_rows: usize) {
    ...
    println!(
        "  压缩速度:     {:.1} MB/s",
        stats_before.total_original as f64 / 1024.0 / 1024.0 / (dur_comp / 1000.0)
    );
    println!();
    ...
    println!(
        "  {:<20} {:>12} {:>12} {:>7.1}%  {:>12?}",
        "id (Int32 自增)",
        fmt_bytes(id_values.len()),
        fmt_bytes(id_comp.len()),
        id_comp.len() as f64 / id_values.len() as f64 * 100.0,
        id_type
    );
    ...
    println!(
        "  {:<20} {:>12} {:>12} {:>7.1}%  {:>12?}",
        "category (Int32 0-99)",
        fmt_bytes(cat_values.len()),
        fmt_bytes(cat_comp.len()),
        cat_comp.len() as f64 / cat_values.len() as f64 * 100.0,
        cat_type
    );
    ...
    println!(
        "  {:<20} {:>12} {:>12} {:>7.1}%  {:>12?}",
        "value (Float64 随机)",
        fmt_bytes(val_values.len()),
        fmt_bytes(val_comp.len()),
        val_comp.len() as f64 / val_values.len() as f64 * 100.0,
        val_type
    );
    ...
    println!(
        "  {:<20} {:>12} {:>12} {:>7.1}%  {:>12?}",
        "name (Varchar 高基数)",
        fmt_bytes(name_data.len()),
        fmt_bytes(name_comp.len()),
        name_comp.len() as f64 / name_data.len() as f64 * 100.0,
        name_type
    );
