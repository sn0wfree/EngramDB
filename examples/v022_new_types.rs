...
fn main() {
    ...
    conn.execute("ALTER TABLE events ADD COLUMN location VARCHAR(200)").expect("ADD COLUMN failed");
    conn.execute("ALTER TABLE events RENAME COLUMN location TO venue").expect("RENAME COLUMN failed");
    conn.execute("ALTER TABLE events ADD COLUMN attendee_count INT").expect("ADD COLUMN failed");
    ...
    // 正常插入
    conn.execute(
        "INSERT INTO products_with_check VALUES (1, '商品A', 99.9, 100)"
    ).expect("Insert failed");
    println!("   正常插入成功：price=99.9, stock=100");

    // 违反 CHECK 约束
    let result = conn.execute(
        "INSERT INTO products_with_check VALUES (2, '商品B', -1.0, 50)"
    );
    match result {
