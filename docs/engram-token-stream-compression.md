...
**正式场景 DB 链路级对比（2026-08-07 补充，107MB 真实语料 / 31830 行 / 三臂同数据同流程）**：

| 维度 | zstd-3 | TokenDelta（三形态 best-of） |
|---|---|---|
| 压缩率 | **14.95×** | 14.95×（持平——best-of 中 TD 未赢过 zstd） |
| checkpoint | **0.39s** | 5.78s（15× 差距） |
| 读取（全扫+校验）| 0.17s | 0.18s（解压非瓶颈，行重组主导） |
| 写入（插入）| 0.15s | 0.15s（压缩不在写路径，无差异） |
| 解码吞吐（codec 级）| ~1GB/s | ~120MB/s（8×） |

**终局决策（数据定稿）**：
1. **运行时压缩主臂 = zstd-3**（无字典；CDict 实测仅 +1% 不引入）——压缩率持平、
   checkpoint 15× 快、解码 8× 快、无增量复杂度
2. **TokenDelta 降级为 config 开关**（`token_delta_enabled`，默认 false）：无 C 依赖
   环境可选；codec 保留（bench/降级）；Tokenizer/词表基建独立保留给 v0.22 检索
3. zstd 转正为 runtime dependency（接受 libzstd-sys C 依赖——项目「零 C 依赖」
   约束仅对压缩路径放开，tokenizers 训练器仍 dev-only）
