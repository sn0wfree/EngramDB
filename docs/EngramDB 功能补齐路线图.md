...
|T11|DATE|🔴 缺失|✅|v0.22.0 新增，i32（自 1970-01-01 起的天数）|
|T12|TIME|🔴 缺失|✅|v0.22.0 新增，i32（自午夜起的毫秒数）|
|T13|DATETIME / TIMESTAMP|🔴 缺失|✅|v0\.14\.0 新增，Unix 毫秒|
|T14|DECIMAL / NUMERIC|🔴 缺失|🔴|仍未实现|
|T15|JSON|🔴 缺失|✅|v0\.12\.0 新增，路径查询|
|T16|JSONB|🔴 缺失|✅|v0.22.0 新增，二进制 JSON 紧凑编码|
|T17|UUID|🔴 缺失|✅|v0.22.0 新增，128 位唯一标识符|
|T18|ENUM / SET|🔴 缺失|✅|v0.22.0 新增，字符串枚举|
|T19|VECTOR\(dim\)|✅ 已有|✅|向量类型（HNSW 索引）|
|T20|VECTOR\_INT8|🔴 缺失|✅|v0\.15\.0 新增，INT8 量化向量|
|T21|ARRAY|🔴 缺失|✅|v0.22.0 新增，同质元素数组|

**小计**：已有 18 项 + 部分 1 项 + 缺失 2 项（完成度 90%）
...
|I09|全文索引 \(FTS5 / BM25\)|🔴 缺失|P1|v0\.16|全文检索，混合排序基础（借鉴 ReasonDB SEARCH BM25）|
...
|V13|搜索 trace（可追溯路径）|🔴 缺失|P1|v0\.15|返回命中节点、分数、索引类型，Agent 溯源刚需（借鉴 ReasonDB）|
|V14|相似度置信度评分|🔴 缺失|P2|v0\.16|0\-1 置信度，Agent 据此判断是否采纳（借鉴 ReasonDB）|
|V15|向量 \+ 全文混合排序 \(RRF\)|🔴 缺失|P1|v0\.16|BM25 \+ 向量相似度融合排序（借鉴 ReasonDB SEARCH\+REASON）|
|V16|VECTOR SIMILAR TO SQL 语法|🔴 缺失|P1|v0\.15|SQL 原生向量查询语法，统一查询接口（借鉴 ReasonDB RQL）|
...
|P11|EXPLAIN / EXPLAIN ANALYZE|P1|v0\.15|查询计划查看（含向量/全文检索阶段）|
...
|Ag06|全文 \+ 向量混合检索|P2|v0\.17|BM25 \+ 向量相似度联合排序（V15 底层支撑）|
...
|Ag11|文档树分层索引|P1|v0\.16|文档转层级树结构，分支节点=摘要，叶子节点=原文（借鉴 ReasonDB HRR）|
|Ag12|文档关系图 \(Relationships\)|P2|v0\.17|REFERENCES / SUPERSEDES / PARENT\_OF 等关系类型（借鉴 ReasonDB）|
|Ag13|插件化摄取管线|P2|v0\.16|Extractor/Chunker/Summarizer/Formatter 四级插件（借鉴 ReasonDB Plugin API）|
|Ag14|多 LLM 提供商抽象层|P1|v0\.16|OpenAI/Anthropic/Ollama/Gemini 一键切换（借鉴 ReasonDB rig\-core）|
|Ag15|语义分块 \+ 节点摘要|P2|v0\.16|基于标题的语义切分 \+ LLM 生成节点摘要（借鉴 ReasonDB 摄入管线）|
|Ag16|文档摄入 API（文件/URL/文本）|P1|v0\.15|一键摄入 PDF/Markdown/URL，自动建索引（借鉴 ReasonDB Ingestion API）|
...
|Eco02|C API \(libengramdb\)|P2|v0\.17|C 接口，便于多语言绑定|
|Eco03|Python 绑定 \(pyo3\)|P1|v0\.16|Python 生态，对接 LangChain|
|Eco04|命令行 CLI \(engramcli\)|P1|v0\.15|交互式查询工具|
...
|Agent 特有|16|0|0|16|100%|
|性能优化|10|0|0|10|100%|
|接口与生态|8|0|0|8|100%|
|**合计**|**240**|**41**|**7**|**192**|**80%**|

> 当前功能完成度约 17%（按项数计）。核心骨架（存储引擎 \+ 事务 \+ 向量 \+ 基础 SQL）已到位，但上层 SQL 功能和 Agent 差异化能力仍有大量补齐空间。新增 10 项 ReasonDB 借鉴功能主要集中在向量检索（\+4）和 Agent 特有（\+6）。
> 
> 
...
**主题**：向量混合查询 \+ TTL \+ 缓存 \+ 限流 \+ 文档摄入，建立 Agent 场景护城河
...
- 文档摄入 API 可用，支持 Markdown/文本一键摄入
...
|V08|INT8 向量量化|中|无|
|V13|搜索 trace（可追溯路径）|小|V03|
|V16|VECTOR SIMILAR TO SQL 语法|中|V03|
|Ag01|内置 TTL（表级）|中|无|
|Ag02|内置 KV 缓存引擎|大|无|
|Ag03|滑动窗口限流原语|中|无|
|Ag07|对话历史时序优化|中|Dt04|
|Ag16|文档摄入 API（文件/文本）|中|T15|
...
- 搜索 trace：返回命中节点 ID、相似度分数、索引类型

- TTL：过期数据在 compaction 时自动清理

- 限流原语：100 万次/秒以上的原子递增

- 文档摄入：Markdown 文本一键摄入，自动建向量索引

- 时间桶聚合查询：比 SQLite 快 10×\+

---

### v0\.16 — 分析能力增强 \+ Agent 深度集成（4\-5 周）

**主题**：JOIN \+ 窗口函数 \+ CTE \+ CBO \+ 文档树 \+ LLM 抽象，分析能力对齐 DuckDB 60%，Agent 能力拉开差距
...
|I09|全文索引 \(FTS5 / BM25\)|大|无|
|I12|REINDEX|小|I03|
|V11|多向量字段|中|V01|
|V14|相似度置信度评分|小|V03|
|V15|向量 \+ 全文混合排序 \(RRF\)|中|I09, V03|
...
|Ag04|分层记忆接口|中|Ag01|
|Ag08|工具调用日志专用表|小|无|
|Ag11|文档树分层索引|大|I09, V03|
|Ag13|插件化摄取管线|大|Ag16|
|Ag14|多 LLM 提供商抽象层|中|无|
|Ag15|语义分块 \+ 节点摘要|中|Ag11, Ag14|
...
- 文档树索引：Markdown 摄入后可按章节树检索

- LLM 抽象：OpenAI/Anthropic/Ollama 三家可切换

- 混合排序：BM25 \+ 向量 RRF 融合，准确率优于单一检索
...
|Agent|记忆重要性评分 \+ 衰减、全文\+向量混合检索、多模态 Blob、任务状态机、文档关系图 \(Relationships\)|
...
|---|---|---|---|---|---|
|**功能点完成数**|41|47|88|142|196|
|**功能完成度**|17%|20%|37%|59%|82%|
|**SQL 兼容性 \(vs SQLite\)**|\~30%|\~35%|\~55%|\~70%|\~85%|
|**OLTP 写入 \(vs SQLite\)**|2%|20%|30%|40%|50%|
|**OLAP 分析 \(vs DuckDB\)**|\~30%|\~30%|\~35%|\~45%|\~60%|
|**Agent 场景就绪度**|低|低|中|高|很高|
|**向量检索能力**|基础 HNSW|基础 HNSW|基础 HNSW|混合查询\+trace\+SQL语法|全文混合\+置信度\+多字段|
|**文档处理能力**|无|无|无|文本摄入 API|文档树\+语义分块\+LLM摘要|
|**可替换场景**|无|日志写入|简单 CRUD|Agent 网关 \+ 记忆|通用嵌入式 \+ Agent 基础设施|

> 注：以上为估算值，需各版本实际测试验证。功能点从 230 增至 240，新增 10 项均来自 ReasonDB 借鉴（向量检索 \+4、Agent 特有 \+6）。
> 
> 
...
|借鉴点|对应功能项|归属版本|价值等级|
|---|---|---|---|
|HRR 分层树检索|Ag11 文档树分层索引|v0\.16|⭐⭐⭐⭐⭐ 核心差异化|
|SEARCH\+REASON 双从句|V15 向量\+全文混合排序 \+ V16 SQL 向量语法|v0\.15/v0\.16|⭐⭐⭐⭐⭐ 统一查询体验|
|可追溯推理路径 \(trace\)|V13 搜索 trace|v0\.15|⭐⭐⭐⭐⭐ Agent 溯源刚需|
|置信度评分|V14 相似度置信度评分|v0\.16|⭐⭐⭐⭐ Agent 决策辅助|
|插件化摄取管线|Ag13 插件化摄取管线|v0\.16|⭐⭐⭐⭐ 生态扩展性|
|多 LLM 提供商抽象|Ag14 多 LLM 提供商抽象层|v0\.16|⭐⭐⭐⭐ 降低接入成本|
|BM25 全文检索|I09 全文索引 \(FTS5/BM25\)|v0\.16|⭐⭐⭐⭐ 混合检索基础|
|文档关系图|Ag12 文档关系图 \(Relationships\)|v0\.17|⭐⭐⭐ 知识图谱能力|
|语义分块 \+ 节点摘要|Ag15 语义分块 \+ 节点摘要|v0\.16|⭐⭐⭐⭐ 检索质量提升|
|Ingestion API|Ag16 文档摄入 API|v0\.15|⭐⭐⭐⭐ 开发者体验|

> **差异化定位**：ReasonDB 是"文档推理引擎"（强依赖 LLM、服务端架构），EngramDB 是"嵌入式 Agent 数据库"（通用 SQL \+ 向量 \+ Agent 原语、零依赖部署）。两者互补而非竞争——EngramDB 吸收 ReasonDB 的检索理念，但保持嵌入式轻量定位。
> 
> 
