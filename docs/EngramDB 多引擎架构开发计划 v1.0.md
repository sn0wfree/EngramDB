...
| 里程碑 | 状态 | 说明 |
|--------|------|------|
| M0：StorageEngine trait + EngineTable 枚举 | ✅ 完成 | v0\.17\.0 实现 |
| M1：主键 Mark Index 持久化 | ✅ 完成 | v0\.17\.0 实现 |
| M2：MemoryEngine | ✅ 完成 | v0\.17\.0 实现，全内存表 |
| M3：LogEngine | ✅ 完成 | v0\.17\.0 实现，追加式时序引擎 |
| M4：WAL 引擎类型字段 | ✅ 完成 | v0\.17\.0 实现，20 字节头 |
| M5：统计信息缓存 | ✅ 完成 | v0\.18 实现 |
| M6：Zone Map | ✅ 完成 | v0\.17\.0 M1\-6 实现 |
| M7：主键索引重建 | ✅ 完成 | v0\.17\.0 M1\-7 实现 |
| M8：Bloom Filter | ✅ 完成 | v0\.17\.0 M1\-8 / P3\.5 实现 |
| P0\-1：查询计划缓存 | ✅ 完成 | v0\.18 P0\-1 实现 |
| P0\-2：INSERT 攒批合并 | ✅ 完成 | v0\.18 P0\-2 实现 |
| P0\-3：时间窗组提交 | ✅ 完成 | v0\.18 P0\-3 实现 |
| P1\-A：列级 Bloom Filter | ✅ 完成 | Phase 2 P1\-A 实现 |
| P1\-B：列级自动编码选择 | ✅ 完成 | Phase 2 P1\-B 实现 |
| P1\-C：LogEngine 跳过 WAL | ✅ 完成 | Phase 2 P1\-C 实现 |
| P2：异步块压缩 | ✅ 完成 | Phase 3 P2 实现 |
| P3：HeatTracker | ✅ 完成 | Phase 2\.5 P1 实现 |
| P4：分层迁移 | ✅ 完成 | Phase 2\.5 P2 / Phase 3 P0 实现 |
| Auto 引擎 | ✅ 完成 | v0\.17\.0 实现，heat\-based 调度 |
| Layered Index | ✅ 完成 | v0\.19 实现，delta 稀密 + 列存稀疏 |
| Constraint Batching | ✅ 完成 | v0\.20 实现，约束表入批预检 |
| TokenDelta 检索引擎 | ✅ 完成 | v0\.21 实现，Token 级倒排 + BM25 + 混合检索 |

### 进行中

| 里程碑 | 状态 | 说明 |
|--------|------|------|
| mmap 统一读路径 | 🟡 部分 | feature = "mmap\-read"，列存热数据走 mmap |
| DataFusion 集成 | 🟡 部分 | feature = "datafusion"，TableProvider 实现 |
| 完整 CBO | 🟡 部分 | RBO + 基础 CBO，统计信息收集待完善 |
