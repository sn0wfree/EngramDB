...
use engramdb::{CompactStrategy, Config, Connection, Value, WalFlushMode};
...
fn bench_with_wal(name: &str, strategy: CompactStrategy, wal_mode: WalFlushMode, total: usize) {
    ...
    println!(
        "  {:<22}  总耗时: {:>8.2} ms  吞吐: {:>7.1}万/s  P50: {:>7.1}µs  P99: {:>8.1}µs  Max: {:>8.2}ms",
        name,
        elapsed.as_secs_f64() * 1000.0,
...
fn bench_periodic_sync_wal(link_compact: bool, total: usize) {
    let name = if link_compact {
        "Periodic+sync_wal联动"
    } else {
        "Periodic+sync_wal独立"
    };
    let path = format!(
        "/tmp/bench_periodic_{}.db",
        if link_compact { "link" } else { "nolink" }
    );
    let _ = std::fs::remove_file(&path);
    let _ = std::fs::remove_file(format!("{}-wal", path));
    ...
    println!(
        "  {:<22}  总耗时: {:>8.2} ms  吞吐: {:>7.1}万/s  查询: {:>6.2} ms",
        name,
        elapsed.as_secs_f64() * 1000.0,
        rows_per_sec / 10000.0,
        q_ms
