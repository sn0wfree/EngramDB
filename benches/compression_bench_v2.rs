...
fn main() {
    ...
    println!(
        ...
        fmt_bytes(bool_orig),
        fmt_bytes(bp.len()),
        ratio(bool_orig, bp.len())
    );
    let unpacked = bool_unpack(&bp);
    assert_eq!(unpacked, bool_sparse, "Boolean 位打包验证失败");
    bench("  BooleanPack 编码", 200, || {
        let e = bool_pack(&bool_sparse);
        std::hint::black_box(e.len());
    });
    bench("  BooleanPack 解码", 200, || {
    ...
    println!(
        "  Delta:         {}  ({:.1}x)",
        fmt_bytes(delta_enc.len()),
        ratio(int_orig, delta_enc.len())
    );
    println!(
        "  FOR+BitPack:   {}  ({:.1}x)",
        fmt_bytes(for_enc.len()),
        ratio(int_orig, for_enc.len())
    );
    println!(
        "  RLE:           {}  ({:.2}x)",
        fmt_bytes(rle_enc.len()),
        ratio(int_orig, rle_enc.len())
    );
    ...
    bench("  Delta 编码", 100, || {
        let e = delta_encode_i64(&int_ts);
        std::hint::black_box(e.len());
    });
    bench("  Delta 解码", 100, || {
        let d = delta_decode_i64(&delta_enc).unwrap();
        std::hint::black_box(d.len());
    });
    bench("  FOR+BitPack 编码", 100, || {
        let e = for_bitpack_encode_i64(&int_ts);
        std::hint::black_box(e.len());
    });
    bench("  FOR+BitPack 解码", 100, || {
        let d = for_bitpack_decode_i64(&for_enc).unwrap();
        std::hint::black_box(d.len());
    });
    println!("  ✓ 全部验证通过");
    ...
    println!(
        "  Delta:         {}  ({:.1}x)",
        fmt_bytes(delta_enc2.len()),
        ratio(int_orig, delta_enc2.len())
    );
    println!(
        "  FOR+BitPack:   {}  ({:.1}x)",
        fmt_bytes(for_enc2.len()),
        ratio(int_orig, for_enc2.len())
    );
    ...
    bench("  Delta 编码", 80, || {
        let e = delta_encode_i64(&int_mono);
        std::hint::black_box(e.len());
    });
    bench("  Delta 解码", 80, || {
        let d = delta_decode_i64(&delta_enc2).unwrap();
        std::hint::black_box(d.len());
    });
    println!("  ✓ 验证通过");
    ...
    println!(
        "  FOR+BitPack:   {}  ({:.1}x)",
        fmt_bytes(for_enc3.len()),
        ratio(int_orig, for_enc3.len())
    );
    println!(
        "  Delta:         {}  ({:.2}x)",
        fmt_bytes(delta_enc3.len()),
        ratio(int_orig, delta_enc3.len())
    );
    println!(
        "  Dictionary:    {}  ({:.2}x)",
        fmt_bytes(dict_sz3),
        ratio(int_orig, dict_sz3)
    );
    let for_dec3 = for_bitpack_decode_i64(&for_enc3).unwrap();
    assert_eq!(for_dec3, int_narrow, "FOR 窄范围验证失败");
    bench("  FOR+BitPack 编码", 100, || {
        let e = for_bitpack_encode_i64(&int_narrow);
        std::hint::black_box(e.len());
    });
    bench("  FOR+BitPack 解码", 100, || {
        let d = for_bitpack_decode_i64(&for_enc3).unwrap();
        std::hint::black_box(d.len());
    });
    println!("  ✓ 验证通过");
    ...
    println!(
        "  RLE:           {}  ({:.1}x)",
        fmt_bytes(rle_enc4.len()),
        ratio(int_orig, rle_enc4.len())
    );
    println!(
        "  Dictionary:    {}  ({:.1}x)",
        fmt_bytes(dict_sz4),
        ratio(int_orig, dict_sz4)
    );
    println!(
        "  Delta:         {}  ({:.1}x)",
        fmt_bytes(delta_enc4.len()),
        ratio(int_orig, delta_enc4.len())
    );
    let rle_dec4 = rle_decode_i64(&rle_enc4, N);
    assert_eq!(rle_dec4, int_repeat, "RLE 高重复验证失败");
    bench("  RLE 编码", 100, || {
        let e = rle_encode_i64(&int_repeat);
        std::hint::black_box(e.len());
    });
    bench("  RLE 解码", 100, || {
        let d = rle_decode_i64(&rle_enc4, N);
        std::hint::black_box(d.len());
    });
    println!("  ✓ 验证通过");
    ...
    println!(
        "  Delta:         {}  ({:.3}x)",
        fmt_bytes(delta_enc5.len()),
        ratio(int_orig, delta_enc5.len())
    );
    println!(
        "  FOR+BitPack:   {}  ({:.3}x)",
        fmt_bytes(for_enc5.len()),
        ratio(int_orig, for_enc5.len())
    );
    println!(
        "  RLE:           {}  ({:.3}x)",
        fmt_bytes(rle_enc5.len()),
        ratio(int_orig, rle_enc5.len())
    );
    ...
    println!(
        "  Gorilla XOR:   {}  ({:.1}x)",
        fmt_bytes(gor_enc.len()),
        ratio(float_orig, gor_enc.len())
    );
    println!(
        "  RLE:           {}  ({:.2}x)",
        fmt_bytes(rle_f1.len()),
        ratio(float_orig, rle_f1.len())
    );
    ...
    bench("  Gorilla 编码", 50, || {
        let e = gorilla_encode(&float_ts);
        std::hint::black_box(e.len());
    });
    bench("  Gorilla 解码", 50, || {
        let d = gorilla_decode(&gor_enc).unwrap();
        std::hint::black_box(d.len());
    });
    println!("  ✓ 验证通过");
    ...
    println!(
        "  Gorilla XOR:   {}  ({:.1}x)",
        fmt_bytes(gor_enc2.len()),
        ratio(float_orig, gor_enc2.len())
    );
    ...
    bench("  Gorilla 编码", 50, || {
        let e = gorilla_encode(&float_lin);
        std::hint::black_box(e.len());
    });
    bench("  Gorilla 解码", 50, || {
        let d = gorilla_decode(&gor_enc2).unwrap();
        std::hint::black_box(d.len());
    });
    println!("  ✓ 验证通过");
    ...
    println!(
        "  Gorilla XOR:   {}  ({:.3}x)",
        fmt_bytes(gor_enc3.len()),
        ratio(float_orig, gor_enc3.len())
    );
    ...
    println!(
        "  Dictionary:    {}  ({:.1}x)  [字典 5 条目 + 索引数组]",
        fmt_bytes(dict_total),
        ratio(vc_low_orig, dict_total)
    );
    ...
    println!(
        "  Dictionary:    {}  ({:.1}x)  [字典 500 条目 + 索引数组]",
        fmt_bytes(dict_total2),
        ratio(vc_med_orig, dict_total2)
    );
    ...
    println!(
        "║  Boolean   稀疏(10% true)   BooleanPack  {:>5.1}x           ║",
        b1_ratio
    );
    println!(
        "║  Boolean   随机 50/50       BooleanPack  {:>5.1}x           ║",
        b2_ratio
    );
