...
impl SkipList {
    fn new() -> Self {
        let head = SkipNode {
            key: i64::MIN,
            value: 0,
            forward: vec![None; SKIP_MAX_LEVEL],
        };
...
fn main() {
    ...
    println!(
        "    → 编码吞吐: {:.1} MB/s  |  {:.2} M 行/秒",
        throughput_mbps(int_orig, ms),
        throughput_mrps(N_BIG, ms)
    );
    ...
    println!(
        "  RLE:        {}  ({:.1}x)",
        fmt_bytes(rle_all.len()),
        int_orig as f64 / rle_all.len() as f64
    );
    println!(
        "  Delta:      {}  ({:.1}x)",
        fmt_bytes(delta_all.len()),
        int_orig as f64 / delta_all.len() as f64
    );
    println!(
        "  FOR+BitPack: {}  ({:.1}x)",
        fmt_bytes(for_all.len()),
        int_orig as f64 / for_all.len() as f64
    );
    ...
    println!(
        "  Delta:      {}  ({:.3}x)  负压缩",
        fmt_bytes(delta_rand.len()),
        int_orig as f64 / delta_rand.len() as f64
    );
    println!(
        "  FOR+BitPack: {}  ({:.3}x)",
        fmt_bytes(for_rand.len()),
        int_orig as f64 / for_rand.len() as f64
    );
    println!(
        "  RLE:        {}  ({:.3}x)  负压缩",
        fmt_bytes(rle_rand.len()),
        int_orig as f64 / rle_rand.len() as f64
    );
    ...
    for &n in &sizes {
        ...
        let dec_ms = {
            let start = Instant::now();
            for _ in 0..iters {
                let d = delta_decode(&enc).unwrap();
                std::hint::black_box(d.len());
            }
            start.elapsed().as_secs_f64() * 1000.0 / iters as f64
        };
        println!(
            "  {:>10}  {:>10.3} ms  {:>9.1} MB/s  {:>10.3} ms  {:>9.1} MB/s",
            fmt_num(n),
            enc_ms,
            throughput_mbps(orig, enc_ms),
            dec_ms,
            throughput_mbps(orig, dec_ms)
        );
    }
    ...
    println!(
        "  原始: {}  Gorilla: {}  压缩率 {:.1}x",
        fmt_bytes(float_orig),
        fmt_bytes(gor_lin.len()),
    ...
    println!(
        "    → 编码吞吐: {:.1} MB/s  |  {:.2} M 行/秒",
        throughput_mbps(bool_orig, ms),
        throughput_mrps(N_BOOL, ms)
    );
